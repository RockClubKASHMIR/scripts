                    ...
                    if marker >= len(homes)-1 {
                        if len(waves) == len(homes) {
                            if HowManyCycles != false {
                                eXpoT = totalExpSlots
                                if sendWhenFleetBack == false && len(homes) > eXpoT {eXpoT = len(homes)}
                                if sendWhenFleetBack == true {eXpoT = len(homes)*totalExpSlots}
                                if CountFleets >= eXpoT {
                                    if Repeat == true && RepeatTimes == 1 {LogInfo("You make "+RepeatTimes+" full cycle of fleet sending")}
                                    if Repeat == true && RepeatTimes > 1 {LogInfo("You make "+RepeatTimes+" full cycles of fleet sending")}
                                    RepeatTimes++
                                    waves = {}
                                    CountFleets = 0
                                }

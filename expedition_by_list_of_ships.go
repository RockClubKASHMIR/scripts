...
homes = ["M:2:485:8"] // Replace coordinates M:1:2:3 with your coordinates. "M" means moon, "P" means planet.
...
SystemsRange = true // Do you want to send the fleets sent on the EXPEDITION mission in a radius of systems around your current solar system? true = YES / false = NO
...
if homeworld != nil {
    CronExec(myTime, func() {
        ...
        for home = current; home <= len(homes)-1; home++ {
                        ...
                        if currentslots < totalUsl {
                            if dflag == 0 {
                                ...
                                if b == nil {
                                    ...
                                    if currentslots == totalUsl {fleetFlag = 1}
            ...
            if ls.InUse == totalUsl {fleetFlag = 1}
            if home >= len(homes)-1 {
                ...
                for slots == totalSlots {
                    ...
                    if Repeat == true {
                        if err != nil {
                            ...
                            if rcyc == 1 || fleetFlag == 1 {delay = totaltime}
                            ...
                            if fleetFlag == 2 {
                                ...
                                if delay > 1 {LogInfo("All slots for Expedition are busy! Please, wait "+ShortDur(delay))}
                            }
                            Sleep(delay*1000)
                    ...
                    if marker >= len(homes)-1 {
                        if len(waves) == len(homes) {
                            if HowManyCycles != false {
                                eXpoT = totalExpSlots
                                if sendWhenFleetBack == false {
                                    if SystemsRange == false && len(homes) > eXpoT {eXpoT = len(homes)}
                                }
                                if sendWhenFleetBack == true {eXpoT = len(homes)*totalExpSlots}
                                if SystemsRange == true {eXpoT = 2*RangeRadius*len(homes)}
                                if CountFleets >= eXpoT {
                                    if Repeat == true {LogInfo("You make "+RepeatTimes+" full cycles of fleet sending")}
                                    RepeatTimes++
                                    waves = {}
                                    CountFleets = 0
                                }
